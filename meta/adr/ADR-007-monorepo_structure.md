...
1. **`packages/core` (`@scrymat/core`):** Contains 100% of the game logic. It knows nothing about browsers, DOM, React, IndexedDB, or AWS S3. It only knows about arrays, numbers, strings, and MTG rules. This includes:
...
2. **`packages/pwa` (`@scrymat/pwa`):** The frontend application. It imports `@scrymat/core` and handles:
...
* **Positive:** `@scrymat/core` becomes a reusable, testable, agent-friendly library. The boundary between "rules" and "presentation" is enforced at the package level, not just by convention. Future MCP/agent integrations can `import { dispatch } from '@scrymat/core'` without pulling in browser dependencies.
...
* **Future Implications:** If the Scryfall layer stabilizes and an agent needs card images, it can be extracted to `@scrymat/scryfall` in a future ADR. The monorepo structure makes this extraction straightforward.
