...
* **Future Implications:** The shuffle module will be a self-contained utility in `@scrymat/core` (`packages/core/src/shuffle.ts`) that can be unit-tested independently. Using `globalThis.crypto` (available in Node.js 19+ and all modern browsers) ensures the module works in both the PWA and any future Node.js-based agent consumer. If the app ever needs other randomness (e.g., coin flips), the same crypto-based utility can be reused.
